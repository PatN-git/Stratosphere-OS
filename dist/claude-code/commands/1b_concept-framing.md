...
- **G1 — Recommend an Answer:** For every question, recommend a candidate answer and explanation; user confirms or corrects.
- **G2 — Facts vs Decisions:** Check codebase for facts first. Do not grill user on discoverable facts; grill only on decisions (preferences, constraints, outcomes).
- **G3 — Dependency-Ordered Grilling:** Resolve decisions sequentially. Probe high-ambiguity axes first.
