...
8. **Terminal sync gate:** run `python .agents/scripts/reconcile.py --ids BT-<slice>[,<each dependent cleared in Step 6>][,BT-<parent> when Step 7 flipped the epic] --pr-id BT-<slice> --fields status,blocked_by` per `references/terminal-sync-invariant.md` (covers the slice + epic `in review` and every dependent's `Blocked by` clear; PR-link checked on the slice only). Non-zero → heal per the reference and re-run until `[MIRROR-OK]`; do not emit the ship line until it passes.
