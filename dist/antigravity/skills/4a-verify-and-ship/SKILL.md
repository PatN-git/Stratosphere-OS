...
> **Named gate — `ship-only`:** this phase alone. Branch-safety + design-drift gate, push, open/update PR, status flip. Never reached by an `audit-only` run.
...
7. **Epic Check:** If **all sibling sub-issues under the parent are `status:in review`** (not "closed" — they close only on merge, which this gate enables; query parent sub-issues using field-scoped `gh issue view <parent> --json subIssues`) → run the **Feature Acceptance Audit** below, then mark the feature PR ready for review (`gh pr ready <n>`, if connected; no-op if it was opened non-draft), state `"All slices for BT-<parent> in review! PR #<n> ready to merge."`, and move the parent epic to `status:in review` (`gh issue edit <parent> --remove-label "status:in progress" --add-label "status:in review"`). The epic and slices move to `status:done` on PR **merge** (auto-close), not here.
   - **Feature Acceptance Audit (once per feature — fires only here, never in `audit-only`).** The branch now holds every slice, so audit the feature as assembled before it becomes mergeable.
     - Invoke one **Strict Business-Logic Auditor** subagent, same guardrail as Phase 2: audit + format the table only; no edits, commits, or pushes.
     ...
     - Scope: **cross-slice integration AC** and **PRD AC unclaimed by any single slice** — what slice-scoped audits structurally cannot see. Do not re-litigate AC already mapped PASS in a shipped slice's table.
     - Output: the Phase 3 §1 AC↔test table, gaps ≥ 80 confidence.
     - **Clean → continue this step.** **Gaps → halt**: leave the PR in draft, leave the epic at `status:in progress`, surface the table, and route to `/3b-create-issue` (new slice) or `/3d-implement-issue` (extend). Never flip `gh pr ready` over an open feature-level gap, and never re-run the audit to clear it.
8. **Terminal sync gate:** run `python .agents/scripts/reconcile.py --require-gh --ids BT-<slice>[,<each dependent cleared in Step 6>][,BT-<parent> when Step 7 flipped the epic] --pr-id BT-<slice> --fields status,blocked_by` per `references/terminal-sync-invariant.md` (covers the slice + epic `in review` and every dependent's `Blocked by` clear; PR-link checked on the slice only). Non-zero → heal per the reference and re-run, **at most 3 attempts**; still non-zero, or `[MIRROR-UNVERIFIED]` → halt and surface the drift. Never loop unbounded; do not emit the ship line until it passes.
9. Output: `[SHIPPED] PR #<n> open for BT-<padded>.` Never merge.