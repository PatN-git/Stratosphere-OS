...
6. **PR-link comment (bi-directional trace):** `gh issue comment <n> --body "Shipped in PR #<pr>"`. The terminal gate (step 9) checks the slice issue for this comment — posting it here is what makes that check pass. Its own step on purpose: folded into the status write below, it is the clause that drops.
...
10. Output: `[SHIPPED] PR #<n> open for BT-<padded>.` Never merge.