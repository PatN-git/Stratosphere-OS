...
- **PRESERVATION RULE:** Do NOT delete or modify operational instructions under Rules (such as Label source of truth syncing rules) or the `Milestone` definition line under Label Registry during setup or backlog updates. The `Area` line under Label Registry must always exist: its slugs are project-defined and may change, but the line itself is never dropped. They must remain permanently as operational guidance.
