...
## 8. Backlog ID Minting & Sequence Governance (Late Binding)
- **Shared Sequence Space:** Because GitHub shares a single monotonic integer sequence counter across both Issues and Pull Requests (`#1`, `#2`, ...), you MUST NEVER pre-allocate, predict, or guess a project issue ID (`BT-<n>`) offline by scanning `.memory/BACKLOG_MAP.md` and calculating `MAX(BT_ID) + 1`. Ephemeral Pull Requests consume sequence numbers that local markdown files do not track.
- **Slug-First Pre-Creation:** During discovery (`/1b_concept-framing`), drafting, and offline planning, items must be identified exclusively by a semantic topic slug (e.g., `app-refactor` or `docs/discovery/app-refactor.md`). Never assign numeric `BT-<n>` padding to pre-creation artifacts.
- **Atomic Minting Protocol:** Numeric ID binding (`BT-<padded>`) occurs strictly at atomic creation time inside `/2a_write-prd` or `/3b_create-issue`. Execute `gh issue create`, capture the exact numeric ID `#N` returned by GitHub, zero-pad it to 3 digits (e.g., `#59` -> `BT-059`), and atomically record it across memory and document frontmatter.
- **Live Sequence Audit / Offline Fallback:** If a tool or script must check the remote sequence head, query the live GitHub API: `max(latest_issue, latest_pr) + 1` via `GET /repos/{owner}/{repo}/issues?state=all&per_page=1`. When operating in offline/local-only mode without GitHub connectivity, use a distinct draft prefix (`BT-LOCAL-<n>`) instead of guessing numbers in the main GitHub sequence space.
