...
def get_skill_metadata(skill_path):
    ...
    # Search for SKILL.md or SKILL_<name>.md
    skill_md = None
    expected_name = f"SKILL_{metadata['id']}.md"
    metadata["standard_name"] = False
    metadata["correct_name"] = expected_name
    
    for pattern in ["SKILL.md", expected_name]:
        candidate = Path(skill_path) / pattern
        if candidate.exists():
            skill_md = candidate
            metadata["standard_name"] = (pattern == expected_name)
            break
