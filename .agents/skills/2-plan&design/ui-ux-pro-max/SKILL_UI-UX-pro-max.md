...
| **Review existing UI** | "Review this page for UX issues", "Check accessibility" | Checklist below |
...
- **Stack**: The user's preferred tech stack
...
`react`, `nextjs`, `vue`, `svelte`, `astro`, `swiftui`, `react-native`, `flutter`, `html-tailwind`, `shadcn-ui`, `jetpack-compose`, `angular`, `laravel`.
...
```bash
python3 .agents/skills/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack <stack>
```
