...
def test_update_flow():
    ...
    repo_root = Path(__file__).resolve().parents[2]
    result = subprocess.run([sys.executable, "tests/framework/test_update_flow.py"], cwd=str(repo_root), capture_output=True, text=True)
    print("Exit code:", result.returncode)
    sys.stdout.flush()
    if result.returncode == 0:
...
def test_update_preflight():
    ...
    repo_root = Path(__file__).resolve().parents[2]
    result = subprocess.run([sys.executable, "tests/framework/test_update_preflight.py"], cwd=str(repo_root), capture_output=True, text=True)
    print("Exit code:", result.returncode)
    sys.stdout.flush()
    if result.returncode == 0:
