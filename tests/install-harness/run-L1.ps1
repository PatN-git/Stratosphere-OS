...
function Assert-ScaffoldTree([string]$proj) {
    ...
    AssertFileCount "scaffold: .agents/workflows/*.md == 19" (Join-Path $proj ".agents\workflows") "*.md" 19
...
function Run-Cell([string]$tool, [string]$scope) {
    ...
    try {
        ...
        if ($tool -eq "claude-code") {
            ...
            AssertFileCount "install: .claude/commands/*.md == 21" (Join-Path $base "commands") "*.md" 21
            ...
            AssertFileCount "install: workflows/*.md == 19" (Join-Path $pluginRoot "workflows") "*.md" 19
