...
assert_scaffold_tree() { # $1 proj
  ...
  assert "scaffold: .agents/workflows 18 md" "$([ "$(nmd "$p/.agents/workflows")" = "18" ] && echo 1 || echo 0)"
...
run_cell() { # $1 tool  $2 scope
  ...
  if [ "$tool" = "claude-code" ]; then
    ...
    assert "install: 20 commands" "$([ "$(nmd "$base/commands")" = "20" ] && echo 1 || echo 0)"
    ...
    assert "install: 18 workflows" "$([ "$(nmd "$plugin/workflows")" = "18" ] && echo 1 || echo 0)"
