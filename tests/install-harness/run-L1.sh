...
assert_scaffold_tree() { # $1 proj
  ...
  assert "scaffold: .agents/workflows 19 md" "$([ "$(nmd "$p/.agents/workflows")" = "19" ] && echo 1 || echo 0)"
...
run_cell() { # $1 tool  $2 scope
  ...
  if [ "$tool" = "claude-code" ]; then
    ...
    assert "install: 21 commands" "$([ "$(nmd "$base/commands")" = "21" ] && echo 1 || echo 0)"
    ...
    assert "install: 19 workflows" "$([ "$(nmd "$plugin/workflows")" = "19" ] && echo 1 || echo 0)"
