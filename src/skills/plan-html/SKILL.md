...
2. Layout requires spatiality or scale:
   - Side-by-side comparison, diagram/flowchart component as a section inside a multi-section plan (branching flow, path routing, state transition map), interactive widget, or multi-section navigation.
...
A lone diagram/flowchart does NOT trigger HTML. A request for just a flow resolves to markdown (optionally with an inline SVG/mermaid), or a `plan-document` diagram section only if it's part of a larger plan. `<5×` size is a sanity ceiling, not the trigger.
