---
name: jules-dispatch
description: EXPERIMENTAL, user-invoked only — offload bounded mode:AFK slices to Google Jules (async cloud agent). Dispatch + report; hands off at "PR opened". Never merges, never orchestrates. Invoke ONLY on an explicit user request (normally via /3x-jules-dispatch) — never autonomously.
version: "0.1.3"
timestamp: 2026-07-15
---

> **User-invoked only.** Run via `/3x-jules-dispatch` or an explicit user request — never
...
it never verifies or merges. You run `/4a-verify-and-ship` and merge.
...
4. **You** verify: `gh pr checkout <pr>` in a real clone → `/4a-verify-and-ship` → merge.
