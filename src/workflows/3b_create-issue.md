...
3.  **Generate (Atomic Minting):** Execute `gh issue create` to create the issue in GitHub (applying the appropriate template below). **CRITICAL:** Capture the exact numeric issue number `#N` returned by GitHub and zero-pad it (`BT-<padded>`). Never guess or compute `MAX(BT_ID) + 1` from local files, as Pull Requests consume IDs from GitHub's shared sequence. Write the raw `ICE`, `Impact`, and `Confidence` inputs directly into the issue body. **Apply the scope label (`scope:baseline` or `scope:differentiator`) based on the slice's inherited scope-class.** (If GitHub is disconnected in local fallback mode, assign ID as `BT-LOCAL-<n>`).
