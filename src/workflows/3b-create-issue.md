...
3. **Pre-mint guard (Template B):** run `grep -nE 'BT-<[^>]+>|\[\[[A-Z]+-[xX]+\]\]' .tmp/BT-<padded>-issue-drafts.md`. Any hit → HALT and list the hits; never mint a placeholder. **Mint order:** if drafts cite sibling slices, state a topological order (blockers first; a cycle → HALT — Blocked-by must be acyclic) and mint in that order. After each mint, substitute the returned `BT-<padded>` for its placeholder in the dependents' drafts and re-run the guard on the next draft before minting it. Substitution is the only permitted edit to an approved draft.
