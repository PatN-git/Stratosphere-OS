...
2. If no external surface/code change → skip; handoff to `/3b_create-issue`.
...
6. Freeze layout into Path A body block. status → `ready-for-slicing`.
...
4. Write blueprint into Path B body block. status → `ready-for-slicing`.
