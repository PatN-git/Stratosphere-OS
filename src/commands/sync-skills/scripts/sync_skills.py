...
def write_pack_gitignore(skills_base, lockfile_path):
    ...
    Regenerated from the lockfile on every run, so a removed pack stops being
    ignored. `.agents/skills/` itself is deliberately NOT ignored - doing so
    would silently drop the bundled skills out of version control.
    """
    names = []
    if lockfile_path.exists():
        try:
            names = sorted(json.loads(lockfile_path.read_text(encoding="utf-8")).get("skills", {}))
        except Exception:
            names = []
    gi = skills_base / ".gitignore"
    lines = ["# Generated by sync_skills.py - on-demand third-party packs.",
             "# Bundled lifecycle skills are tracked and must not appear here.", ""]
...
def main():
    ...
    if not args.dry_run:
        packs = write_pack_gitignore(skills_base, lockfile_path)
        print(f"[gitignore] {len(packs)} on-demand pack(s) ignored; bundled skills tracked")
