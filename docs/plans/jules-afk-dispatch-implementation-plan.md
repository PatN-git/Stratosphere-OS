...
status: deprecated  # was: COMPLETE — P0–P7 done; live E2E passed and torn down 2026-07-16
...
3. **Cross-host launcher — BUILT.** A thin bundled workflow `src/workflows/x_jules-dispatch.md` delegates to the skill (no logic of its own — §1 workflow→skill delegation, §2 no duplication). `sync_skills.py` installs only to `.agents/skills/<name>` and can't place a workflow, so the launcher is bundled in `src/workflows/` instead: Claude Code registers the plugin command globally; for Antigravity, `scaffold.py` copies it into the project's `.agents/workflows/` (`x_jules-dispatch.md` added to `EXTRA_WORKFLOWS`, unified with `map_bundled_to_project`). Net: a discoverable **user-only** `/x_jules-dispatch` on both hosts + a stable call target for future workflow→skill delegation. Trade-off: a thin launcher is baked into the framework (shipped to all projects); the execution pack stays opt-in via sync-skills.

**🚦 Gate P6:** README experimental section + data-egress note present; `build/validate.py` guard asserts no `experimental/` path in `dist/`; `python build/validate.py` = OK; `/x_jules-dispatch` present in both `dist/*/commands|workflows` and mapped by `scaffold` into `.agents/workflows/`.
