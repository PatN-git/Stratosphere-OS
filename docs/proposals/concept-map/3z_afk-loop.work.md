...
- **Orchestrator rule:** invokes other workflows by command name; never duplicate bodies (AGENTS.md §1). `3z` is sole AFK/autonomous orchestrator of the **build** lifecycle (`0a`→`3d`→`4a`→`0b`). `1c` is the user-invoked discovery orchestrator and owns its own drain; `3z` may call its **bounded AFK surface** `/1c_concept-map --drain <map>` (AGENTS.md §1) but never `1c`'s charting, working, or convergence phases — those are HITL.
