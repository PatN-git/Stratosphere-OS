...
## 1. Architecture
- **Layer 1: Workflows** (`.agents/workflows`) -> Human-In-The-Loop (HITL) processes (e.g., Discover, Design, Implement, Review). These dictate the step-by-step lifecycle and are triggered only by the user. Do not execute them autonomously. A user-invoked **orchestrator** workflow may sequence other workflows as part of its authorized run. A workflow may additionally expose one **bounded AFK surface** (a named flag, e.g. `/1c_concept-map --drain`) that an authorized orchestrator or a scheduled routine may call without a user present. These two are the only sanctioned forms of workflow-invoking-workflow.
  - **Bounded AFK surface — all five hold, or it is not one:** (1) it acts only on work labelled `mode:AFK`; (2) it never resolves, closes, or approves `mode:HITL` work — it may *prepare* it (look up facts, build an artifact, post a proposal) and must leave it open; (3) it never passes a hard HITL gate, writes a `.memory/` content entry, or merges; (4) every unit it touches reaches a stated terminal state, attempts are bounded, and exhaustion **demotes** the unit to `mode:HITL` rather than retrying or dropping it; (5) it restates its guardrails at the boundary and reports what it did. Work carrying **no** mode label is never AFK-eligible.
