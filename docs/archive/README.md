...
## 🏗️ Installed System Architecture & Flow

Once installed via `/stratosphere-setup`, StratosphereOS transforms your project into a **state-aware agentic workspace**. It separates durable knowledge from ephemeral loop work to keep LLM context token-light while maintaining 100% architectural recall.

### 1. Spatial Anatomy (Directory & State Topology)

```mermaid
graph TD
    %% Root Workspace
    subgraph Root["📁 Target Workspace Root"]
        direction TB
        Const["📜 Constitution<br/>(AGENT.md / CLAUDE.md / GEMINI.md)"]
        
        subgraph Agents["📁 .agents/ (Orchestration Engine)"]
            Rules["📐 /rules<br/>• output-mode.md<br/>• memory-protocol.md"]
            Workflows["⚡ /workflows<br/>• /0a_start-session → /4b_drift-audit"]
            Skills["🧠 /skills<br/>• micro-tdd, plan-html<br/>• external: db, web, mobile, design"]
        end

        subgraph Memory["📁 .memory/ (Authoritative State)"]
            Status["🟢 STATUS.md<br/>(Session & Build Health)"]
            Backlog["📋 BACKLOG_MAP.md<br/>(Taxonomy & BT-xxx Registry)"]
            Learnings["💡 LEARNINGS.md<br/>([[L-xxx]] Brownfield Traps)"]
            Glossary["📖 GLOSSARY.md<br/>([[G-xxx]] Domain Terms)"]
            Arch["🏛️ ARCHITECTURE.md<br/>([[A-xxx]] System Invariants)"]
            DBSchema["🗄️ DATABASE_SCHEMA.md<br/>([LAW] Live DB Schema)"]
            Design["🎨 DESIGN.md & DESIGN_RULES.md<br/>(Brand Tokens & [[DR-xxx]] Law)"]
        end

        subgraph Docs["📁 docs/ (OKF Knowledge Bundle)"]
            PRDs["📄 /prds & /discovery"]
            Roadmap["🗺️ ROADMAP.md"]
        end

        Tmp["🗑️ .tmp/ (Ephemeral Handoffs & Logs)"]
    end

    Const -->|Enforces Precedence| Agents
    Agents -->|Reads & Updates| Memory
    Memory -->|Contextualizes| Docs
```

### 2. Developer Session Flow (Lifecycle & Inner Loops)

```mermaid
sequenceDiagram
    autonumber
    actor Dev as Loop Engineer (Human)
    participant Orch as Layer 2: Orchestration Brain
    participant Mem as .memory/ Layer
    participant Code as Layer 3: Inner Loop (TDD / Git)

    %% Session Start
    Dev->>Orch: /0a_start-session
    Orch->>Mem: Read STATUS.md, BACKLOG_MAP.md, LEARNINGS.md
    Mem-->>Orch: Dense Durable Context
    Orch->>Code: Restore active git branch (feat/BT-xxx)
    Orch-->>Dev: Synced Session Ready

    %% HITL Discovery & Design
    rect rgb(30, 40, 60)
        Note over Dev,Orch: Layer 1: HITL Outer Loop (Discover & Design)
        Dev->>Orch: /1a Research & /1b Concept Framing
        Orch->>Mem: Accrue [[G-xxx]] Glossary & Discovery Brief
        Dev->>Orch: /2a Write PRD & /2b Interface Design
        Orch->>Mem: Write PRD & [[DR-xxx]] UI Contracts
        Dev->>Orch: /3b Create Issue (Slice PRD)
        Orch->>Mem: Register vertical slices BT-xxx
    end

    %% Autonomous Implementation
    rect rgb(20, 50, 40)
        Note over Orch,Code: Layer 3: Autonomous Inner Loop (/3d Implement)
        Dev->>Orch: /3d_implement-issue (BT-xxx)
        loop Micro-TDD Execution
            Orch->>Code: 1. Write failing Red test
            Code->>Code: 2. Execute test suite (Capture Failure Log)
            Code->>Code: 3. Implement Green surgical code
            Code->>Code: 4. Refactor & local git commit
        end
        Code-->>Orch: Slice Complete (All Tests Green)
    end

    %% Ship & Close
    Dev->>Orch: /4a_verify-and-ship
    Orch->>Code: Audit AC vs test coverage & push branch
    Code-->>Dev: Open traceable Pull Request
    Dev->>Orch: /0b_stop-session
    Orch->>Mem: Run validate_memory.py & update STATUS.md
    Orch-->>Dev: Clean Session Handoff
```

#### Workflow Step Rationale (What & Why)

| Lifecycle Stage | Command | What it does | Why it exists |
|:---|:---|:---|:---|
| **0. Session Start** | `/0a_start-session` | Reads `.memory/STATUS.md`, `BACKLOG_MAP.md`, and `LEARNINGS.md`, restores your feature branch (`feat/BT-xxx`), and syncs agent context. | Eliminates expensive LLM cold starts and context re-reading by loading dense, pre-digested durable state. |
| **1. Discovery** | `/1a_research`<br/>`/1b_concept-framing` | Investigates domain context, seeds terms into `GLOSSARY.md`, and deconstructs fuzzy ideas into a structured discovery brief. | Ensures problem-space alignment between human and agent before speculative architecture or code is written. |
| **2. Design** | `/2a_write-prd`<br/>`/2b_interface-design` | Converts discovery briefs into a formal PRD and derives brand tokens (`DESIGN.md`) and UI structural contracts (`DESIGN_RULES.md`). | Establishes unambiguous blueprints and contracts, preventing runaway agent hallucination and UI drift. |
| **3. Issue Slicing** | `/3b_create-issue` | Deconstructs the PRD into traceable vertical-slice issues (`BT-xxx`) registered in `BACKLOG_MAP.md`. | Enforces small, atomic units of work that an autonomous inner loop can reliably complete within one context window. |
| **3d. Execution** | `/3d_implement-issue` | Executes the autonomous `micro-tdd` loop: writes a failing Red test, runs the test suite, writes Green code, refactors, and commits. | Replaces manual prompt-and-fix chat with deterministic, self-correcting compiler/linter feedback loops. |
| **4. Ship** | `/4a_verify-and-ship` | Audits acceptance criteria against automated test coverage, pushes local commits, and opens a traceable Pull Request. | Enforces production-grade quality gate and guarantees 100% traceability from original PRD to git commit. |
| **0. Session Stop** | `/0b_stop-session` | Lints memory (`validate_memory.py`), updates `STATUS.md` and `LEARNINGS.md`, and writes an ephemeral handoff file. | Self-anneals project memory so the next developer session resumes from pristine ground truth. |
...
>
> **2. Check dependencies.** StratosphereOS needs **Python** (to run the project scaffolder and the skill-sync script). The Antigravity path additionally needs **Git** (to clone). We also recommend **GitHub CLI (`gh`)** for automated backlog tracking. Run `python --version`, `git --version`, and `gh auth status`. If any are missing or unauthenticated, use your native question tool — `AskUserQuestion` on Claude Code, `ask_question` on Antigravity — to ask the user whether to install/authenticate them and explain why. Only on confirmation, install via the system package manager (`winget` on Windows, `brew` on macOS, `apt`/`dnf` on Linux). **Never install system software silently.**
>
